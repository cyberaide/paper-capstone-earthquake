...
\acmBooktitle{Report for the MLCommons Science Working Group in collaboration with University of Virginia, May 2022, Charlottesville, VA}
...
\begin{comment}
\parindent 0pt {\bf Citation:}

\begin{footnotesize}
\begin{verbatim}@TechReport{mlcommons-eartquake,
...
This makes benchmarking using a scientific dataframe an ideal candidate
...
different scientific dataframes, in particular score the models based on
...
of that particular scientific field. By using verified dataframes
...
Currently MLCommons Science Research has four dataframes based on
climate, material science, medicine and earthquakes with a fifth
dataframe on plasma physics being added soon. Each dataframe has their own
...
\subsection{Spatial bag Time Series}
...
$1950-2020$. Each location point is $0.1$ by $0.1$ degrees or $11 km$ by $11
...
spatial position, and time. The data was accumulated into $2$ week samples. 
\cite{fox2021earthquake}

\subsection{Averaging Data equations}

The data was filtered so only quakes with a $magnitude > 3.29$, or significant 
quakes, were considered. The Log Energy equation (\ref{eq:Log Energy}) was used for 
the magnitude of significant quakes inside a $2$ week period. The Log Energy 
equation maximizes the largest earthquake energy values for the $2$ week period
\cite{fox2021earthquake}.


    \begin{equation}
    m_{bin} = \log (Energy) = \frac{1}{1.5} \log_{10} \sum\limits^{Events}_{in\ bin} 10^{1.5m_{event}}
    \label{eq:Log Energy}
    \end{equation}
    
\TODO{Gregor suggests to change the event to e(1.5m) Gregor's suggestion just to show it see \ref{eq:log-energy-gregor}. possibly we should use  $in\ bin(1.5m)$ and than just $10^e$ ?. However we need to confirm.}


    \begin{equation}
    m_{bin} = \log (Energy) = \frac{1}{1.5} \log_{10} \sum\limits^{Events}_{e \in bin} 10^{e(1.5m)}
    \label{eq:log-energy-gregor}
    \end{equation}
    
    \begin{equation}
    m_{bin} = \log (Energy) = \frac{1}{1.5} \log_{10} \sum\limits^{Events}_{e \in bin(1.5m)} 10^e
    \label{eq:log-energy-gregor}
    \end{equation}
    
The Energy Weighting equation (\ref{eq:Energy Weighted}) was used to average depth from quakes \cite{fox2021earthquake}.

    \begin{equation}
    Energy\ weighted\ Quantity \ Q_{bin} = \dfrac{\sum\limits^{Events}_{in\ bin} 10^{1.5m_{event}}Q_{event}}{\sum\limits^{Events}_{in\ bin}10^{1.5m_{event}}}
    \label{eq:Energy Weighted}
    \end{equation}

The Multiplicity equation (\ref{eq:Multiplicity}) is used to count the number of 
occurrences of significant earthquakes in a 2 week period \cite{fox2021earthquake}. 

    \begin{equation}
    Multiplicity_{bin} = \sum^{Events}_{in\ bin} Multiplicity_{event}
    \label{eq:Multiplicity}
    \end{equation}


All input values and predictions are independently normalized and have a maximum 
modulus of $1$ over all space and time values. \cite{fox2022aiforscience}

\subsection{Fault Groupings}

There are hundreds of faults in the $32$ to $36$ lat $-120$ to $-114$ long area. To simplify these faults, this area was taken as a $2d$ structure spit into $2400$ individual pixels. These pixels were grouped empirically into $36$ families by proximity using $4$ different space filling curves to fill in empty spaces.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/Fault_groupings.png}
    \caption{Fault Regions}
\end{figure}

\subsection{Training - Test split}

From the $2400$ pixels in the $2d$ image, the $500$ most active pixels were picked. These $500$ were picked according to the magnitude > $3.29$ cut. $400$ were used for training and $100$ for validation and testing.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/Training_test.png}
    \caption{Training - Test Split: $400$ Training points are shown in red and $100$ test/validation points are shown in green}
\end{figure}
...
\TODO{either put plots here or reference where they are}

The models are measured according to two accuracy metrics, the Nash Sutcliffe Efficiency 
($NSE$) and a visualization of predicted verse observed earthquake data. $NSE$ measures 
the variation difference between observed and predicted data over time. $Q^t_m$ is the 
model prediction at time t, $Q^t_o$ is observed value at time $t$, $\overline{Q_o}$ is the 
mean observed value over all time \cite{fox2021earthquake}.

    \begin{equation}
    NSE = 1 - \dfrac{\sum\limits^{t=T}_{t=1}(Q^t_m-Q^t_o)^2}{\sum\limits^{t=T}_{t=1}(Q^t_o-\overline{Q_o})^2}
    \label{eq:Nash Sutcliffe Efficiency}
    \end{equation}
...
The results are presented as the Normalized Nash Sutcliffe Efficiency ($NNSE$) \cite{fox2021earthquake}
$NNSE. NNSE = 1/(2-NSE)$
The values of $NNSE$ ranges between $0$ and $1$ with $0 = mean$, the higher the value the better the fit \cite{fox2021earthquake}.
...
\caption{Gregors MNIST Benchmarks}\label{tab:mnist}
    ...
    \hline
        ...
        Rob K & \\ \hline
        Thomas AERO 15 & 0m3.387s & 0m00.000s & 0m0.015s & 511.79 & 11.6 & NVIDIA RTX 2070 & June 1, 2019 & Intel Core i7-8750H @ 2.20GHz & Q2  2018 \\ \hline
    \end{tabular}
    \end{adjustbox}
\end{table*}
...
\TODO{Rivanna description needs review}

UVA, Rivanna, A100, RTX3090, others may not have enough memory.
\cite{www-rivanna} \TODO{Write paragraph describing Rivanna compute cluster}
...
Sample Text about GPU Event Times

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/GPU/event_times.png}
    \caption{GPU Event Times: Nvidia V100 2 Epoch Experiment on Rivanna localscratch}
\end{figure}

Sample Text about Epoch Validation Figure

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/GPU/validation.png}
    \caption{Epoch Validation: Nvidia V100 2 Epoch Experiment on Rivanna localscratch}
\end{figure}

Sample Text about kWh/Epoch

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/GPU/kWh_per_epoch.png}
    \caption{Average kWh per Epoch: }
\end{figure}
...
%\begin{adjustbox}{angle=90}
\begin{adjustbox}{max width=\textwidth}
    \begin{tabular}{|r|l|l|l|r|r|r|l|r|l|l|l|}
        \hline
        See also & 
        Organization   & 
        Machine                         &
        ...
        \hline
         ...
         Feb 2022   \\
         \hline
         \ref{sec:tcomputer}   & Thomas.A  & i7-8750H & Win10 Pro  & 1 &   6 & 12 & \href{https://www.gigabyte.com/Laptop/AERO-15--RTX-20-Series#kf}{NVIDIA GeForce RTX 2070 with Max-Q Design}  & 16GB & 1 & 1 & 2019   \\
         \hline
         \ref{sec:t2computer}   & Thomas.B  & i7-7700HQ &  Win10 Pro &1 & 4 & 8 &  NVIDIA GeForce GTX 1060  & 16GB & 1 & 1 & 2017   \\
         \hline
         Table \ref{tab:rcompute} & 
         Rob & 
         5900HX & 
         Win11 Pro & 
         1 & 
         8 & 
         16 & 
         \href{https://rog.asus.com/us/laptops/rog-strix/2021-rog-strix-scar-15-series/}{NVidia RTX 3080} &  
         16GB & 
         1 & 
         1 & 
         11/25/2021 \\
...
\BIOFIG{images/bio/jake.jpg}{Jake Kolessar}{is a Graduate Student at
...
\BIOFIG{images/bio/j-fox.jpg}{Judy Fox} is an Associate Professor of Data  
  Science and holds a joint appointment as Associate Professor with the
  Department of Computer Science. Her research focuses on designing
  experimental systems that enable Data Science, Machine Learning, and  the
  Internet  of  Things  applications  to  harness  the  computational
  resources  of  Cloud  and  HPC  platforms  effectively.
...
\begin{itemize}
    \item Introduction to Python \cite{las-intro-python}, Gregor von Laszewski, with contributions from Robert Knuuti
    \item \href{https://laszewsk.github.io/mlcommons/docs/tutorials/rivanna/}{Running GPU Batch jobs on Rivanna} Gregor von Laszewski, Robert Knuuti
\end{itemize}

\TODO{There are more we developed in the web page} 


% \markdownInput{rivanna-manual}
...
\item created a portable version of python 3.10.2 that can run on Rivanna \GITPULL{8}
\item implemented cloudmesh-data (\href{https://github.com/cloudmesh/cloudmesh-data/pull/1}{cloudmesh/cloudmesh-data PR 1}, \href{https://github.com/cloudmesh/cloudmesh-data/pull/3}{cloudmesh/cloudmesh-data PR #3})
